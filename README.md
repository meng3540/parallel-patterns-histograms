...
Histograms are a convienient way to show the frequecy of occurance of a particular data point in a data set. This can be used in image processing to view the distribution of brightness values in an image. In audio processing, histograms can be used to see the distribution of frequencies (after FFT) in a signal. In Quality Assurance, histograms can be used to see the distribution of data to determine if a linearization algorithm is required.
